...
function mappingLabel(mapping) {
    if (!mapping || !mapping.kind)
        return null;
    if (mapping.kind === "Statement" || mapping.kind.Statement)
        return "stmt";
    if (mapping.kind === "Virtual" || mapping.kind.Virtual)
        return "virtual";
    if (mapping.kind.InlineCallEnter)
        return `enter:${mapping.kind.InlineCallEnter.callee}`;
    if (mapping.kind.InlineCallExit)
        return `exit:${mapping.kind.InlineCallExit.callee}`;
    if (mapping.kind.Synthetic)
        return `syn:${mapping.kind.Synthetic.label}`;
    return null;
}
...
function editorLineMetrics() {
    const editor = $("editor");
    if (!editor)
        return { lineHeight: 24, paddingTop: 8 };
...
function getOpcodeSteps(trace) {
    if (!trace)
        return [];
    if (Array.isArray(trace.opcode_steps) && trace.opcode_steps.length)
        return trace.opcode_steps;
...
function getSourceSteps(trace) {
    if (!trace)
        return [];
    if (Array.isArray(trace.source_steps) && trace.source_steps.length)
        return trace.source_steps;
    if (Array.isArray(trace.steps))
        return trace.steps;
    return [];
}
...
function frameIdFromStep(step) {
    if (!step)
        return null;
    if (step.frame_id != null)
        return Number(step.frame_id);
    if (step.mapping && step.mapping.frame_id != null)
        return Number(step.mapping.frame_id);
    return null;
}
function frameSpanForFrame(steps, frameId) {
    ...
    for (const step of steps || []) {
        ...
        if (!mapping || !mapping.span)
            continue;
        const curFrameId = frameIdFromStep(step);
        if (curFrameId !== target)
            continue;
...
function getActiveStepIndex(state, steps) {
    ...
    if (state.dbg.mode === "opcode")
        return clamp(state.dbg.opcodeStep || 0, 0, max);
...
function stepDepth(step) {
    if (!step)
        return 0;
    if (step.call_depth != null)
        return Number(step.call_depth);
    if (step.mapping && step.mapping.call_depth != null)
        return Number(step.mapping.call_depth);
    return 0;
}
...
function syncOpcodeFromSource(state) {
    ...
    const opcodeSteps = getOpcodeSteps(trace);
    const sourceSteps = getSourceSteps(trace);
    if (!opcodeSteps.length || !sourceSteps.length)
        return;
...
function sourceStepOverIndex(steps, idx) {
    ...
    for (let i = idx + 1; i < steps.length; i++) {
        if (stepDepth(steps[i]) <= currentDepth)
            return i;
    }
...
function resetDbgCursor(state) {
    ...
    if (sourceSteps.length && opcodeSteps.length)
...
function renderEditorGutter(state) {
    ...
    for (let i = 1; i <= lines; i++) {
        ...
        if (activeLine === i)
            cls.push("active");
        if (state.dbg && state.dbg.breakpoints && state.dbg.breakpoints.has(i))
            cls.push("bp");
...
function renderRunForm(state) {
    ...
    if (!outline) {
        ...
        if (el("selectorMeta"))
            el("selectorMeta").textContent = "selector: —";
        if (el("sigscriptMeta"))
            el("sigscriptMeta").textContent = "sigscript: —";
        if (el("ctorArgsView"))
            el("ctorArgsView").innerHTML = `<div class="muted">—</div>`;
        if (el("fnArgsView"))
            el("fnArgsView").innerHTML = `<div class="muted">—</div>`;
        if (el("sigscriptHex"))
            el("sigscriptHex").textContent = "—";
        if (el("sigscriptHint"))
            el("sigscriptHint").textContent = "";
        return;
    }
    ...
    if (!state.run.function || !names.has(state.run.function))
        state.run.function = fns.length ? fns[0].name : "";
    if (fnSelect)
        fnSelect.value = state.run.function || "";
    ...
    if (el("selectorMeta"))
        el("selectorMeta").textContent = outline.without_selector ? "selector: none (single entrypoint)" : selectorIndex == null ? "selector: —" : `selector: #${selectorIndex}`;
    ...
    if (el("runMeta"))
        el("runMeta").textContent = `${fns.length} entrypoints`;
    updateRunButton(state);
}
...
async function refreshSigscript(state) {
    ...
    try {
        ...
        const el = (id) => document.getElementById(id);
        if (el("sigscriptHex"))
            el("sigscriptHex").textContent = payload.sigscript_hex ? `0x${payload.sigscript_hex}` : "—";
        if (el("sigscriptHint"))
            el("sigscriptHint").textContent = payload.sigscript_hex ? bytesHint(payload.sigscript_hex) : "";
        if (el("sigscriptMeta"))
            el("sigscriptMeta").textContent = payload.sigscript_len != null ? `sigscript: ${payload.sigscript_len} bytes` : "sigscript: —";
        if (payload.selector_index != null && !payload.without_selector) {
            if (el("selectorMeta"))
                el("selectorMeta").textContent = `selector: #${payload.selector_index}`;
        }
    }
    catch (err) {
        const el = (id) => document.getElementById(id);
        if (el("sigscriptHex"))
            el("sigscriptHex").textContent = "—";
        if (el("sigscriptHint"))
            el("sigscriptHint").textContent = String(err.message || err);
        if (el("sigscriptMeta"))
            el("sigscriptMeta").textContent = "sigscript: —";
    }
}
async function runTrace(state, { openDebug } = {}) {
    ...
    try {
        ...
        if (trace && trace.meta && trace.meta.sigscript_hex) {
            const el = (id) => document.getElementById(id);
            if (el("sigscriptHex"))
                el("sigscriptHex").textContent = `0x${trace.meta.sigscript_hex}`;
            if (el("sigscriptHint"))
                el("sigscriptHint").textContent = bytesHint(trace.meta.sigscript_hex);
            if (el("sigscriptMeta"))
                el("sigscriptMeta").textContent = `sigscript: ${trace.meta.sigscript_len} bytes`;
        }
...
function continueToBreakpoint(state) {
    ...
    for (let k = start + 1; k < sourceSteps.length; k++) {
        ...
        if (line && state.dbg.breakpoints.has(line))
            return k;
    }
    return last;
}
...
function renderDbg(state) {
    ...
    const sourceSteps = getSourceSteps(trace);
    const opcodeSteps = getOpcodeSteps(trace);
    if (state.dbg.mode === "source" && !sourceSteps.length)
        state.dbg.mode = "opcode";
    if (state.dbg.mode === "opcode" && !opcodeSteps.length)
        state.dbg.mode = sourceSteps.length ? "source" : "opcode";
    const steps = getActiveSteps(state);
    if (!steps.length) {
        updateEditorDecorations(state, null, null);
    ...
    const sequence = snap.sequence != null ? snap.sequence : snap.mapping && snap.mapping.sequence != null ? snap.mapping.sequence : null;
    ...
    const statusBits = [];
    if (sequence != null)
        statusBits.push(`seq=${sequence}`);
    if (frame != null)
        statusBits.push(`frame=${frame}`);
    statusBits.push(`depth=${depth}`);
    setStatus(`${modeLabel} step ${step}/${steps.length - 1} | pc=${snap.pc} | off=${snap.byte_offset} | ${statusBits.join(" | ")} | ${snap.last_opcode ? "last=" + snap.last_opcode : "start"}${err ? " | ERROR: " + err : ""}${!snap.is_executing && !err ? " | SCRIPT PASSED" : ""}`, `${line ? "line " + line : "no span"}${label ? " | " + label : ""}${callStack.length ? " | call: " + callStack.join(" > ") : ""}`);
    ...
    renderVars("dbgVarsView", "dbgVarsMeta", snap.vars || [], err, `${modeLabel}${callStack.length ? " | call: " + callStack.join(" > ") : ""}`);
    renderDbgBps(state);
    scrollOpsToStep();
...
function renderDbgOps(state) {
    ...
    const activeOpcodeStep = state.dbg.mode === "opcode"
        ? clamp(state.dbg.opcodeStep || 0, 0, opcodeSteps.length - 1)
        : clamp(Number((getSourceSteps(trace)[clamp(state.dbg.sourceStep || 0, 0, Math.max(0, getSourceSteps(trace).length - 1))] || {}).pc || 0), 0, opcodeSteps.length - 1);
    ...
    for (const op of trace.opcodes) {
        ...
        if (op.index === activeOpcodeStep + 1)
            classes.push("next");
        const badges = [];
        if (line)
            badges.push(`<span class="badge">${escapeHtml("L" + line)}</span>`);
        if (label) {
            const cls = label === "stmt" ? "badge statement" : label === "virtual" ? "badge statement" : label.startsWith("syn:") ? "badge synthetic" : "badge";
            badges.push(`<span class="${cls}">${escapeHtml(label)}</span>`);
        }
        rows.push(`<div class="${classes.join(" ")}" data-opindex="${op.index}">
...
function renderStack(targetId, items) {
    ...
    for (let i = 0; i < reversed.length; i++) {
        ...
        rows.push(`<div class="stack-item" title="Click to copy">
...
function setupLayoutSplitter() {
    ...
    const restore = () => {
        ...
        const n = Number(raw);
        if (Number.isFinite(n) && n > 0)
            apply(clampInt(n, MIN_WIDTH, MAX_WIDTH));
    };
    restore();
    splitter.addEventListener("pointerdown", (e) => {
        ...
        const current = Number.parseInt(getComputedStyle(root).getPropertyValue("--inspector-width"), 10) ||
        ...
        const onUp = () => {
            ...
            const finalWidth = Number.parseInt(getComputedStyle(root).getPropertyValue("--inspector-width"), 10) || startWidth;
...
async function main() {
    ...
    $("btnClearKeys").addEventListener("click", () => {
        if (!state.keys.length)
            return;
        if (!window.confirm(`Delete all ${state.keys.length} key(s)?`))
            return;
        state.keys = [];
        saveKeys(state.keys);
        renderKeyWallet(state);
    ...
    $("helpOverlay").addEventListener("click", (e) => {
        if (e.target && e.target.id === "helpOverlay")
            toggleHelp(state, false);
    });
    // Debug tabs
    ...
    $("editorGutter").addEventListener("click", (e) => {
        ...
        if (!line)
            return;
        if (state.dbg.breakpoints.has(line))
            state.dbg.breakpoints.delete(line);
        else
            state.dbg.breakpoints.add(line);
        renderEditorGutter(state);
        renderDbgBps(state);
    });
...
main().catch((err) => {
    console.error(err);
    document.body.innerHTML = `<pre style="padding:16px;font-family:ui-monospace,monospace;">Failed to load: ${escapeHtml(String(err && err.message ? err.message : err))}</pre>`;
});
